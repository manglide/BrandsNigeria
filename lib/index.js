...
function jax(pid, callback) {
  const sqlComments = "SELECT likes AS likes, dislikes AS dislike, rating AS rate, " +
        " user_comments AS comment, user_location_lat AS latitude, user_location_lon AS longitude," +
        " date AS datePublished FROM product_review WHERE product_id = '" + pid + "' ";
  connection.query(sqlComments, function(error, resultC) {
          if (error) throw error;
          const commentsall = resultC.map((item, index) => (
            '<div class="aboutProductComments" itemprop="review" itemscope itemtype="http://schema.org/Review">'+
            '<div class="individualUserAvatarAndName">'+
            '<div class="avatar">'+
            '<img src="https://asknigeria.com.ng/brands/images/blank_boy.png" width="30" height="30" />'+
  ...
  const sql = "SELECT all_products.id AS product_ID, all_products.title AS productname,all_products.about AS description, all_products.manufacturer AS manufacturer, SUM(product_review.likes) AS likes, SUM(product_review.dislikes) AS dislikes, " +
          ...
          connection.query(sql, function (error, result) {
            ...
            const block2 = result.map((item, index) => (
              ...
              '<div class="productpagebTagCategory" itemprop="category" itemscope itemtype="http://schema.org/manufacturer">'+ // productpagebTagCategory Begins
              item.category.toUpperCase() +
              '</div>'+ // productpagebTagCategory Ends
              '<div class="productpagebTag" itemscope itemtype="http://schema.org/manufacturer">BY</div>'+
              '<div class="productpagebTag" itemprop="manufacturer" itemscope itemtype="http://schema.org/manufacturer">' +
              ...
              ' / 5 stars</span><br />'+
              '<span class="numofCustReviews" itemprop="reviewCount">'+item.usercomments+' customer reviews</span>'+
              ...
              item.fullcomments.split(",").map((item_U, index_U) => (
                ...
                '</div></div></div>'+
                '<div class="DateX">'+
                '<meta itemprop="datePublished" content="'+item_U.split("#")[6]+'" />'+
                '<span itemprop="datePublished">'+item_U.split("#")[6]+'</span>'+
                '</div></div></div>'
              )).join('')+
              '</div>'+
              ...
              '</a></div>'+
              '<div id="AreaAcceptanceMapElem" data="'+item.product_ID+'" class="card-block collapse in" style="height: 300px">'+
              ...
              '</a></div>'+
              '<div id="AreaRejectionMapElem" data="'+item.product_ID+'" class="card-block collapse in" style="height: 300px">'+
