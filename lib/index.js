...
/*
function handleRender(req, res, val) {
...
app.get('/', function (req, res) {
  ...
  connection.query(sql, function (error, result) {
    ...
    const block1 = result.map((item, index) => (
      ...
      Array(Math.floor(item.rating)).fill(2).map((item, index) => (
        "<span class='fa fa-star fa-1x' />"
      )).join('')+
      // (item.rating % 1 === 0) ? "<span />" : "<span class='fa fa-star-half-full' />"
      "</div>"+
