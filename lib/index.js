...
function renderFullPage(html) {
  return `<div class='ne'>${html}</div>`;
}
/*
function handleRender(req, res) {
  // Render the component to a string.
  // res.locals.rating = req.params.ratingVal;
  console.log(req.ratingVal);
...
var handleRender = function (req, res, next) {
  console.log('Request Val:', req.param.val);
  res.send(renderToString(<StarRatingComponent
                          name={'Rating'}
                          starColor="#F60437"
                          emptyStarColor="#000"
                          editing={false}
...
app.use('/ratings/:val', handleRender);
*/
/*
+renderToString(<StarRatingComponent
  name={'Rating'}
  starColor='#F60437'
  emptyStarColor='#000'
  editing={false}
  starCount={5}
  value={item.rating}
  renderStarIcon={(indexT, valueT) => {return <span class={indexT <= valueT ? 'fa fa-star' : 'fa fa-star-o'} />;
  }}
  renderStarIconHalf={() => <span class='fa fa-star-half-full' />} />)+
