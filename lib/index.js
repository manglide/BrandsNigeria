...
app.get('/', function (req, res) {
  ...
  connection.query(sql, function (error, result) {
    ...
    res.render('index', {con: block + block1 + '</div>', meta: meta, keywords: keywords, itemList: List + List2 + List3});
  });
});
...
app.post('/productrecommendation', function (req, res) {
  ...
  connection.query(sqlComments, function(error, resultUp) {
          ...
          const sqlReco = "SELECT all_products.id AS product_ID, all_products.title AS productname,all_products.about AS description, all_products.manufacturer AS manufacturer, SUM(product_review.likes) AS likes, SUM(product_review.dislikes) AS dislikes, " +
                "CASE WHEN (product_review.likes) > (product_review.dislikes) THEN 'trending_up' ELSE 'trending_down' END AS `trend`, " +
                "CASE WHEN (product_review.likes) > (product_review.dislikes) THEN 'Up' ELSE 'Down' END AS `trend_direction`," +
                "CASE WHEN (product_review.likes) > (product_review.dislikes) THEN 'sentiment_very_satisfied' ELSE 'sentiment_very_dissatisfied' END AS `sentiment`, " +
                "CASE WHEN (product_review.likes) > (product_review.dislikes) THEN 'Good' ELSE 'Bad' END AS `sentiment_mood`, " +
                ...
                "JOIN product_categories ON all_products.category = product_categories.id " +
                "WHERE all_products.about <> '' AND all_products.title = '" + resultUp[0].productname + "' " +
                ...
                connection.query(sqlReco, function (error, result) {
                  ...
                  if (!result.length) {
                    ...
                    const sql_ = "SELECT all_products.id AS product_ID, all_products.title AS productname,all_products.competitor_1 AS firstCompetition, " +
                          ...
                          " JOIN product_categories ON all_products.category = product_categories.id " +
                          " WHERE all_products.title = '" + resultUp[0].productname + "' ";
...
app.get('/product/:id', function (req, res) {
  const sql = "SELECT all_products.id AS product_ID, all_products.title AS productname,all_products.about AS description, all_products.manufacturer AS manufacturer, SUM(product_review.likes) AS likes, SUM(product_review.dislikes) AS dislikes, " +
          ...
          " product_review.date AS datePublished, "+
          "GROUP_CONCAT(" +
		           "CONCAT_WS('#', product_review.likes, product_review.dislikes, product_review.rating, product_review.user_comments, product_review.author, product_review.user_location_lat, product_review.user_location_lon, product_review.date)" +
	        ") AS fullcomments," +
          ...
          connection.query(sql, function (error, result) {
            const richSnippet = result.map((item, index) => (
              '{'+
                  ...
                  '"aggregateRating": { "@type": "AggregateRating", "ratingValue": "'+item.rating+'", "reviewCount": "'+item.usercomments+'" },'+
                  '"offers": { "@type": "Offer", "priceCurrency": "NGN", "price": "'+item.price+'" },'+
                  '"review": ['+
                  item.fullcomments.split(",").map((item_U, index_U) => (
                    '{"@type": "Review", "itemReviewed": {"@type": "Thing","name": "'+item.productname.replace(/\n/g, '')+'"},'+
                    '"author": {"@type": "Person", "name": "'+item.author+'"},"datePublished": "'+new Date(item_U.split("#")[6])+'",'+
            ...
            /*const block = '<div class="productWrapper"><div class="adsenseBanner" id="adsenseBanner">'+
                          '<ins class="adsbygoogle" '+
                          'style="display:block" '+
                          'data-ad-client="ca-pub-6672536950725842" '+
                          'data-ad-slot="4537524011" '+
            ...
            const block2 = result.map((item, index) => (
              ...
              item.fullcomments.split(",").map((item_U, index_U) => (
                ...
                '<div class="UserAndComment">'+
                '<div>'+
                '<span class="individualComment" itemprop="author"><b>'+item_U.split("#")[4]+'</b></span>'+
                '</div>'+
